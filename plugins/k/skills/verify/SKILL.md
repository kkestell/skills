---
name: verify
description: Verify completed work against its plan or brainstorm, run the repo's quality gates, invoke supporting skills when needed, and confirm merge readiness before `/k:end-task`.
...
1. Run this after implementation and before `/k:end-task`.
...
3. Read `<verification_target> $ARGUMENTS </verification_target>`, then build the verification surface.
   ...
   - If the repo is dirty, do not recommend `/k:end-task`.
...
4. Invoke supporting skills when relevant.
   - Prefer `/k:rust-lang` for changed `*.rs` files and `/k:deslop` for changed user-facing docs.
...
7. Write the final report using `${CLAUDE_SKILL_DIR}/assets/verification-report-template.md`.
   ...
   - If clean and blocker-free, offer `/k:end-task`.
   - Otherwise summarize what must be fixed or whether `/k:commit` is needed first.

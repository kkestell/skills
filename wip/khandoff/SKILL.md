...
**Workspace directory:** `docs/dev/` at the repository root — all handoff files
live here.
...
2. List the existing files under `docs/dev/handoff/` to find the highest
   sequence number already used today, then increment by one. Format: `NNN`
...
4. Use the filename: `docs/dev/handoff/YYYY-MM-DD-NNN-<slug>.md` where NNN is
   the largest existing NNN plus one for that specific day (e.g. 2026-03-23-001,
   2026-03-23-002, 2026-03-24-001, etc). Create the directory if it does not
   exist. `docs/dev/` is tracked in git — do not add it to `.gitignore` or
   `.git/info/exclude`.
...
5. Write the handoff document to the confirmed path. It should include:
   ...
   Match the style and depth of the existing handoff docs in
   `docs/dev/handoff/`.
